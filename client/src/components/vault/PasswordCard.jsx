 ...
 * 
 * Displays a card for a credential (password, credit card, or secure note) with:
 * - Encryption/decryption of sensitive data
 * - Password strength indicator
 * - Favorite toggling
 * - 2FA code display
 * - Copy to clipboard functionality
 * - Edit, archive, restore, and delete operations
 * - Attachment viewing
 * 
 * @param {Object} credential - The credential object to display
 * @param {Function} onCredentialDeleted - Callback when credential is deleted
 * @param {Function} onCredentialUpdated - Callback when credential is updated
 * @param {Array} listPasswords - List of all passwords (for edit modal)
 * @param {Function} setListPasswords - State setter for password list
 */
const PasswordCard = ({ credential, onCredentialDeleted, onCredentialUpdated, listPasswords, setListPasswords }) => {
  ...
  // Get the icon component for this credential, default to Globe
  ...
  /**
   * Toggle favorite status of credential
   * Optimistically updates UI, then makes API call
   * Reverts on failure
   */
  ...
  /**
   * Restore archived credential
   * Also restores associated 2FA if present
   */
  const handleRestore = async (idParam, ownerIdParam) => {
    ...
    try {
      // If credential has 2FA, restore that first
   ...
   * Supports: content, password, cardNumber, cvv
   */
  ...
  /**
   * Toggle 2FA code display
   */
  const handleToggle2FA = () => setIsShow2FA(!isShow2FA)

  /**
   * Delete or archive credential
   * @param {string} state - 'soft' for archive, 'deleted' for permanent deletion
   * 
   * Handles 2FA deletion/archival based on state
   */
  const handleDelete = async (state = 'soft') => {
    try {
      // Handle 2FA deletion/archival if present
      if(credential.has2fa){
        ...
        if(state === 'soft'){
          ...
          // Permanently delete 2FA
          await ApiService.deleteTotpEntry(totpId);
        }
      }
      
      // Delete/archive the credential
      ...
      notifyCredentialsMutated({ 
        source: 'PasswordCard', 
        kind: state === 'deleted' ? 'delete' : 'archive', 
        credentialId: credId 
      })
  ...
  /**
   * Decrypt credential data on mount or when credential/vaultKey changes
   * Also extracts password strength score if available
   */
  useEffect(() => {
    const decryptData = async () => {
      ...
      try {
        ...
        // Extract password strength if available
  ...
  /**
   * Get password strength badge configuration
   * Only applies to login credentials with passwords
   * @param {number} score - Password strength score (0-100)
   * @returns {Object|null} Badge config with label and className
   */
  ...
  // Error state if decryption failed
  ...
  return (
    <>
      ...
      <div className="w-full hover:shadow-lg bg-white border border-gray-200 rounded-lg px-3 py-4 flex flex-col">
        ...
        {/* Header Section: Icon, Title, Badges, and Menu */}
        <div className="flex flex-col md:flex-row md:justify-between gap-3">
          
          {/* Left Side: Icon and Title with Badges */}
          ...
          {/* Right Side: Last Update and Menu */}
                ...
                <div className={isArchived ? 'absolute' : 'absolute bg-white left-0 sm:left-auto sm:right-0 sm:top-auto sm:mt-2 border border-gray-200 shadow-lg w-44 rounded-lg z-50'}>
                  ...
                  {isArchived && (
                    <div className="absolute bg-white left-full top-0 ml-2 sm:left-auto sm:right-0 sm:top-auto sm:mt-2 border border-gray-200 shadow-lg w-36 rounded-lg z-50">
                      {/* Restore Button */}
                      <button 
                        onClick={() => handleRestore(credential.id, user?.id)} 
                        className="w-full text-left text-sm text-gray-700 hover:bg-black rounded-t-lg pl-2 hover:text-white flex gap-x-2 py-1.5 items-center"
                      >
                  ...
                  {/* Edit Button (non-archived only) */}
                  {!isArchived && (
                    <button 
                      className="w-full text-left text-sm text-gray-700 hover:bg-black pl-2 hover:text-white flex gap-x-2 py-1.5 items-center" 
        ...
        <div className="flex flex-wrap sm:flex-nowrap text-sm gap-x-3 gap-y-2 mt-1 md:mt-3 text-gray-500 items-center">
          <div className="flex items-center gap-x-2 sm:gap-x-3 flex-wrap sm:flex-nowrap">
            ...
            {/* Credit Card Category: Show Card Number and CVV */}
          ...
          {/* Action Buttons: Show/Hide and Copy */}
        ...
        {/* 2FA Code Display (when toggled on) */}
        {isShow2FA && (
      ...
      {/* Delete Confirmation Modal (for archived items) */}

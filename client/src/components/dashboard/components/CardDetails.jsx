...
// import decryptAesGcmBrowser from '../../../utils/crypto.js'; // adjust path
// import '../style/card-details.css';

// // Import the necessary icons for the original UI
// import visibilityToggleEye from '../../../assets/icons/dashboard-icons/visibility-toggle-eye.svg';
// import clipboardCopy from '../../../assets/icons/dashboard-icons/clipboard-copy.svg';
// import passwordEdit from '../../../assets/icons/dashboard-icons/password-edit.svg';

// export default function CardDetails({ isOpen, onClose, itemId }) {
...
//   // Logic to fetch item remains the same
//   useEffect(() => {
//     if (!isOpen) {
//       setItem(null);
//       setDecrypted(null);
...
//       setError(null);
//       try {
//         const res = await fetch(`http://localhost:4000/api/credentials/${itemId}`, {
...
//         setError('Network error while fetching item.');
//         setItem(null);
//       } finally {
//         setLoading(false);
//       }
...
//     fetchDetail();
//   }, [isOpen, itemId, userId]);

//   // Logic to resolve encrypted fields and decrypt remains the same
//   function resolveEncFields(itemObj) {
//     if (!itemObj) return null;
//     const ct = itemObj.dataEnc ?? itemObj.ciphertext ?? itemObj.ct ?? itemObj.data_enc;
//     const iv = itemObj.dataIv ?? itemObj.iv ?? itemObj.data_iv;
...
//     }
//     if (!item) {
//       setError('No item loaded yet.');
//       return;
//     }

//     const enc = resolveEncFields(item);
//     if (!enc) {
//       console.error('Missing ciphertext/iv/tag fields on item:', item);
//       setError('Server response missing encrypted fields (iv/ciphertext/authTag).');
//       return;
//     }
...
//       let parsed;
//       try { parsed = JSON.parse(plain); } catch { parsed = plain; }
//       // We assume the actual password/login is in the decrypted data
//       setDecrypted(parsed);
//       setRevealed(true);
//     } catch (err) {
//       console.error('decrypt error', err);
//       setError('Unable to decrypt. Ensure vault is unlocked with the correct key.');
//     }
//   }

//   // --- UI Adaptations ---

//   // Helper to get the tag object for the current item
//   const itemTag = useMemo(() => {
//     if (!item) return null;
//     if (item.compromised) return { text: 'Exposed', variant: 'exposed' };
//     if (item.passwordReused) return { text: 'Reused', variant: 'reused' };
//     if (item.passwordStrength === 'Weak') return { text: 'Weak', variant: 'weak' };
//     return null;
//   }, [item]);

//   // Use a derived data structure to simulate the old 'item.passwords' array
//   // This maintains the original component's mapping structure for styling
//   const passwordsToDisplay = useMemo(() => {
//     if (!item) return [];
    
//     // Simulate the expected password object for the original UI
//     const passwordObject = {
//       id: item.id,
//       title: item.title,
//       // The decrypted data might be an object, use a common key (e.g., 'username' or 'login') or the item title
//       login: (typeof decrypted === 'object' && decrypted?.username) || (typeof decrypted === 'object' && decrypted?.login) || item.category, 
//       password: decrypted, // The actual decrypted value (or null/placeholder)
//       site: item.title, // Use title for the site name
//       tag: itemTag,
//     };
    
//     // The original component mapped over an array, so we create an array of one item
//     return [passwordObject];

//   }, [item, decrypted, itemTag]);

//   if (!isOpen) return null;

//   // We are now combining the original UI structure with the new state/logic
//   return (
//     <div className="card-details-overlay">
//       <div className="card-details-container">
//         <div className="card-details-header">
//           <div className="details-title">
//             {/* You could add a site-icon here if your item data includes it */}
//             <h2>{item ? (item.title ?? 'Details') : 'Loading...'}</h2>
//           </div>
//           <button className="close-button" onClick={onClose} aria-label="Close details">
//             &times;
//           </button>
...
//           {/* Display loading/error/empty state */}
//           {loading && <div>Loading...</div>}
//           {error && <div style={{ color: 'crimson', padding: '10px 0' }}>{error}</div>}
          
//           {/* Main content: show password list or empty state */}
//           {item && passwordsToDisplay.length === 0 ? (
//              <div className="empty-state">
//                <p>No item details available.</p>
//              </div>
//           ) : (
//              // Revert to the original map structure but use the simulated array
//             passwordsToDisplay.map(pwd => (
//               <div key={pwd.id} className="password-item">
//                 <div className="site-info">
//                   {/* Site Icon placeholder - assuming your item has an 'icon' field */}
//                   <div className="site-icon">
//                     <img src={item.icon ?? 'placeholder-url'} alt="" width="24" height="24" />
//                   </div>
...
//                       // If decrypted is an object, show 'Decrypted' or JSON.stringify(decrypted)
//                       value={revealed ? (typeof decrypted === 'string' ? decrypted : JSON.stringify(decrypted, null, 2)) : '••••••••'}
//                       readOnly 
...
//           {/* If the decrypted content is a complex object, it will be JSON stringified in the input field, 
//               which may not be ideal. The original structure didn't account for this, 
//               but the logic above handles it by putting the JSON into the input value. */}
